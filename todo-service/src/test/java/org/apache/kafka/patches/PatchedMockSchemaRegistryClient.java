...
/**
 * Mock implementation of SchemaRegistryClient that can be used for tests. This version is
 * NOT thread safe. Schema data is stored in memory and is not persistent or shared across
 * instances.
 *
 * Patch for unreleased fix in 7.0.2
 *
 * @see <a href=
 ...
 */
public class PatchedMockSchemaRegistryClient extends MockSchemaRegistryClient implements SchemaRegistryClient {

	private static final Logger log = LoggerFactory.getLogger(PatchedMockSchemaRegistryClient.class);
	...
	public PatchedMockSchemaRegistryClient() {
		this(null);
	}

	public PatchedMockSchemaRegistryClient(List<SchemaProvider> providers) {
	...
	@Override
	public Optional<ParsedSchema> parseSchema(String schemaType, String schemaString,
	...
	private int getIdFromRegistry(String subject, ParsedSchema schema, boolean registerRequest, int id)
			throws RestClientException {
		Map<Integer, ParsedSchema> idSchemaMap = idCache.computeIfAbsent(subject, (k) -> new ConcurrentHashMap<>());
		...
		if (registerRequest) {
			...
			Map<ParsedSchema, Integer> schemaIdMap = schemaIdCache.computeIfAbsent(context,
					(k) -> new ConcurrentHashMap<>());
			Integer schemaId = schemaIdMap.computeIfAbsent(schema, (k) -> (id >= 0) ? id
	...
	private void generateVersion(String subject, ParsedSchema schema) {
		...
		Map<ParsedSchema, Integer> schemaVersionMap = versionCache.computeIfAbsent(subject,
				(k) -> new ConcurrentHashMap<>());
	...
	private int register(String subject, ParsedSchema schema, int version, int id, boolean normalize)
		...
		Map<ParsedSchema, Integer> schemaIdMap = schemaCache.computeIfAbsent(subject, (k) -> new ConcurrentHashMap<>());
		...
		synchronized (this) {
			...
			String context = toQualifiedContext(subject);
			final Map<Integer, ParsedSchema> idSchemaMap = idCache.computeIfAbsent(context,
					(k) -> new ConcurrentHashMap<>());
	...
	public ParsedSchema getSchemaBySubjectAndId(String subject, int id) throws IOException, RestClientException {
		...
		final Map<Integer, ParsedSchema> idSchemaMap = idCache.computeIfAbsent(subject,
				(k) -> new ConcurrentHashMap<>());
	...
	public List<ParsedSchema> getSchemas(String subjectPrefix, boolean lookupDeletedSchema, boolean latestOnly)
			throws IOException, RestClientException {
		Stream<String> validSubjects = getAllSubjects().stream().filter((subject) -> subject.startsWith(subjectPrefix));

		return validSubjects.flatMap((subject) -> getSchemasForSubject(subject, latestOnly))
				.collect(Collectors.toList());
	}

	@Override
	public Collection<String> getAllSubjectsById(int id) throws IOException, RestClientException {
		return idCache.entrySet().stream().filter((entry) -> entry.getValue().containsKey(id)).map(Map.Entry::getKey)
				.collect(Collectors.toCollection(LinkedHashSet::new));
	}

	@Override
	public Collection<SubjectVersion> getAllVersionsById(int id) throws IOException, RestClientException {
		return idCache.entrySet().stream().filter((entry) -> entry.getValue().containsKey(id)).map((e) -> {
	...
	@Override
	public synchronized List<Integer> deleteSubject(Map<String, String> requestProperties, String subject,
	...
	@Override
	public synchronized Integer deleteSchemaVersion(Map<String, String> requestProperties, String subject,
			String version, boolean isPermanent) throws IOException, RestClientException {
		Map<ParsedSchema, Integer> schemaVersionMap = versionCache.get(subject);
		if (schemaVersionMap != null) {
			for (Map.Entry<ParsedSchema, Integer> entry : schemaVersionMap.entrySet()) {
	...
	private static String toQualifiedContext(String subject) {
		QualifiedSubject qualifiedSubject = QualifiedSubject.create(QualifiedSubject.DEFAULT_TENANT, subject);
